...
Original file is located at
    https://colab.research.google.com/drive/1DLzrCAbdJdFuBJl4D8MErLb_NFgE59Ba
"""
...
from build_vocab import create_vocabulary
...
ABBR_CATEGORIES = ['Agri', 'Air', 'Base', 'Beach', 'Bldg', 'Chap', 'DenRes', 'For', 'Free', 'Golf',
                   'Harb', 'Int', 'MedRes', 'MHP', 'Over', 'Park', 'Riv', 'Run', 'SpaRes', 'StorTanks', 'Tenn']
...
def main():
    ...
    if FEATURE == 'tiny_image':
        ...
        if os.path.isfile('vocab_150.pkl') is False:
            print('No existing visual word vocabulary found. Computing one from training images\n')
            vocab_size = 150 ### Vocab_size is up to you. Larger values will work better (to a point) but be slower to comput.
            vocab = create_vocabulary(train_image_paths, vocab_size)
            with open('vocab_150.pkl', 'wb') as handle:
                pickle.dump(vocab, handle, protocol=pickle.HIGHEST_PROTOCOL)

        if os.path.isfile('train_image_feats_150.pkl') is False:
            # YOU CODE get_bags_of_sifts.py
            train_image_feats = get_bags_of_sifts(train_image_paths);
            with open('train_image_feats_150.pkl', 'wb') as handle:
                pickle.dump(train_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('train_image_feats_150.pkl', 'rb') as handle:
                train_image_feats = pickle.load(handle)

        if os.path.isfile('test_image_feats_150.pkl') is False:
            test_image_feats  = get_bags_of_sifts(test_image_paths);
            with open('test_image_feats_150.pkl', 'wb') as handle:
                pickle.dump(test_image_feats, handle, protocol=pickle.HIGHEST_PROTOCOL)
        else:
            with open('test_image_feats_150.pkl', 'rb') as handle:
